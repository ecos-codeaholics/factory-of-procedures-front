"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var http_1 = require('@angular/http');
var Observable_1 = require('rxjs/Observable');
require('rxjs/Rx');
var CitizenService = (function () {
    function CitizenService(http) {
        this.http = http;
        this.citizensUrl = 'http://localhost:4567/citizens/';
    }
    CitizenService.prototype.handleError = function (error) {
        var errMsg = (error.message) ? error.message :
            error.status ? error.status + " - " + error.statusText : 'Server error';
        console.error(errMsg);
        return Observable_1.Observable.throw(errMsg);
    };
    CitizenService.prototype.extractData = function (res) {
        var body = res.json();
        return body.data || {};
    };
    CitizenService.prototype.getCitizen = function (id) {
        return this.getCitizens()
            .toPromise()
            .then(function (citizens) { return citizens.find(function (citizen) { return citizen.id === id; }); });
    };
    CitizenService.prototype.getCitizens = function () {
        var req = new http_1.Request({
            method: "GET",
            url: this.citizensUrl
        });
        return this.http.request(req)
            .map(function (res) { return res.json(); });
    };
    CitizenService = __decorate([
        core_1.Injectable(), 
        __metadata('design:paramtypes', [http_1.Http])
    ], CitizenService);
    return CitizenService;
}());
exports.CitizenService = CitizenService;
//# sourceMappingURL=citizen.service.js.map