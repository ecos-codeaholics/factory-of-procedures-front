"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var angular2_jwt_1 = require('angular2-jwt');
var JwtHelperService = (function () {
    function JwtHelperService(jwtHelper) {
        this.jwtHelper = jwtHelper;
        this.token = localStorage.getItem('id_token');
    }
    JwtHelperService.prototype.tokenDecode = function () {
        if (!this.token) {
            return "unathorized";
        }
        else {
            return this.jwtHelper.decodeToken(this.token);
        }
    };
    JwtHelperService.prototype.tokenExpirationDate = function () {
        return this.jwtHelper.getTokenExpirationDate(this.token);
    };
    JwtHelperService.prototype.tokenExpired = function () {
        return this.jwtHelper.isTokenExpired(this.token);
    };
    JwtHelperService = __decorate([
        core_1.Injectable(), 
        __metadata('design:paramtypes', [angular2_jwt_1.JwtHelper])
    ], JwtHelperService);
    return JwtHelperService;
}());
exports.JwtHelperService = JwtHelperService;
//# sourceMappingURL=jwt-helper.service.js.map